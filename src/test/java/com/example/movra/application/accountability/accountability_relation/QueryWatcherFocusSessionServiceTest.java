...
class QueryWatcherFocusSessionServiceTest {
    ...
    void queryRange_fromAfterTo_throwsException() {
        given(watcherAccountabilityRelationReader.getCurrentWatcherRelation())
                .willReturn(relationWith(MonitoringTarget.FOCUS_SESSION));

        assertThatThrownBy(() -> queryWatcherFocusSessionService.queryRange(
                LocalDate.of(2026, 4, 15),
                LocalDate.of(2026, 4, 14)
        )).isInstanceOf(InvalidDateRangeException.class);

        verifyNoInteractions(dailyFocusSummaryReader);
    }
