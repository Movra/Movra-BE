...
class QueryWatcherTimetableTaskServiceTest {
    ...
    @BeforeEach
    void setUp() {
        queryWatcherTimetableTaskService = new QueryWatcherTimetableTaskService(
    ...
    void queryRange_fromAfterTo_throwsException() {
        given(watcherAccountabilityRelationReader.getCurrentWatcherRelation())
                .willReturn(relationWith(MonitoringTarget.TIMETABLE_TASK));

        assertThatThrownBy(() -> queryWatcherTimetableTaskService.queryRange(
                LocalDate.of(2026, 4, 15),
                LocalDate.of(2026, 4, 14)
        )).isInstanceOf(InvalidDateRangeException.class);

        verifyNoInteractions(dailyTimetableSummaryReader);
    }
