...
| 비교 항목 | 캐시 유지 비용 | 캐시 제거 이득 |
| --- | --- | --- |
| 코드 복잡도 | `@Cacheable` × 3, `@CacheEvict/@CachePut` × 7개 메서드 = **총 10개 어노테이션** | 어노테이션 전량 제거 |
| Evict 누락 리스크 | 변경 경로 6개 서비스에 분산 → 누락 시 stale 응답 | 리스크 없음 |
| Redis 의존성 | 홈 조회가 Redis 장애에 영향 받음 | 영향 없음 |
| 직렬화 공격 표면 | Jackson polymorphic deserialization 경로 활성 | 홈 조회 경로 제거 |
| p95 지연시간 | 74 ms (데이터 미설정 계정), 개선폭 불명확 | **66–77 ms (측정 노이즈 내 동일)** |
| p99 안정성 | 105 ms | 124 ms (JVM 워밍 시) — **19ms 차이는 구조적 이득 아님** |

목표 부하(10 TPS)에서 캐시 없이 p95 66–77ms / 실패 0%로 충분하다.
캐시가 제공하는 p99 안정성(−19ms 워밍 기준)은 위의 유지 비용 대비 이득이 없다.
